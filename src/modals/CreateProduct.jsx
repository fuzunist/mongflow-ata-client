...
import { addProductToDB, delProductFromDB, editProductToDB } from '@/services/product'
...
const CreateProduct = ({ closeModal, selectedProduct, type }) => {
    ...
    const validate = (values) => {
        ...
        if (values.attributes.length === 0) errors.attributes = 'Add at least one feature. 1'
        if (values.attributes.some((attr) => !attr.attribute_name)) errors.attributes = 'Add at least one feature. 2'
        if (values.attributes.some((attr) => attr.values.length === 0)) errors.attributes = 'Add at least one feature. 3'
        if (values.attributes.some((attr) => attr.values.some((val) => !val))) errors.attributes = 'Add at least one feature. 4'
        return errors
    }
    ...
    const onDelete = async () => {
        console.log('product editing: del')
        setError('')
        const response = await delProductFromDB(user.tokens.access_token, selectedProduct.product_id)
        if (response?.error) return setError(response.error)
    ...
    return (
        <>
            <FormikForm
                ...
                error={error}
                title={t(selectedProduct ? (!!type ? 'editOtherProduct' : 'editProduct') : !!type ? 'addOtherProduct' : 'addProduct')}
            />
            {selectedProduct && (
                <button
