...
export const generateRandomOrderNumber = () => {
    ...
    let suffix
    let counterValue
    if (usertype === 'domestic_market_manager' || usertype === 'domestic_market_marketing') {
        suffix = 'B'
        counterValue = daily_domestic_order_counter++
    } else if (usertype === 'foreign_market_manager' || usertype === 'foreign_market_marketing') {
        suffix = 'A'
    ...
    return `${formattedDate}-MG-${counterValue.toString().padStart(3, '0')}-${suffix}`
}
...
export const isEmptyArray = (item) => {
    return item && typeof item === 'object' && Array.isArray(item) && !item.length
}
...
export const imageToBase64 = (urlImg, cropped) => {
    return new Promise((resolve) => {
        ...
        img.onload = () => {
            ...
            cropped
                ? ctx.drawImage(img, cropped.x, cropped.y, cropped.width, cropped.height, 0, 0, canvas.width, canvas.height)
