...
export const _promiseAll = createAsyncThunk('apps/promiseAll', async ({ access_token, usertype }, { rejectWithValue }) => {
    let [products, sets, customers, orders, stocks, productions, users, exchangeRates] = await Promise.all([
    ...
    if (products?.error) return rejectWithValue({ type: 'getProductsFromDB', error: products.error })
    else if (sets?.error) return rejectWithValue({ type: 'getSetsFromDB', error: sets.error })
    else if (customers?.error) return rejectWithValue({ type: 'getCustomersFromDB', error: customers.error })
    else if (orders?.error) return rejectWithValue({ type: 'getOrdersFromDB', error: orders.error })
    else if (stocks?.error) return rejectWithValue({ type: 'getStocksFromDB', error: stocks.error })
    else if (productions?.error) return rejectWithValue({ type: 'getProductionsFromDB', error: productions.error })
    else if (users?.error) return rejectWithValue({ type: 'getUsersFromDB', error: users.error })
    else if (exchangeRates?.error) return rejectWithValue({ type: 'getTodayExchangeRates', error: exchangeRates.error })
...
const apps = createSlice({
    ...
    reducers: {
        _addStock: (state, action) => {
            state.stocks = [...state.stocks, action.payload].sort((a, b) => new Date(a.date) - new Date(b.date))
        },
        _editStock: (state, action) => {
            state.stocks = state.stocks.map((stock) => {
                if (stock.stock_id === action.payload.stock_id) stock = { ...stock, ...action.payload }
                return stock
            })
        },
        _delStock: (state, action) => {
            state.stocks = state.stocks.filter((stock) => stock.stock_id !== action.payload)
        },
        _addProduction: (state, action) => {
            state.productions = [...state.productions, action.payload].sort((a, b) => new Date(a.date) - new Date(b.date))
        },
        _editProduction: (state, action) => {
            state.productions = state.productions.map((production) => {
                if (production.production_id === action.payload.production_id) production = { ...production, ...action.payload }
                return production
            })
        },
        _delProduction: (state, action) => {
            state.productions = state.productions.filter((production) => production.production_id !== action.payload)
        },
        ...
        _editCustomer: (state, action) => {
            state.customers = state.customers.map((customer) => {
                if (customer.customerid === action.payload.customerid) customer = action.payload
                return customer
            })
        },
        ...
        _delOrder: (state, action) => {
            state.orders = state.orders.filter((order) => order.order_id !== parseInt(action.payload))
        },
        _setOrderStatus: (state, action) => {
            const { order_id, index, productOrderStatus, orderStatus } = action.payload
            state.orders = state.orders.map((order) => {
        ...
        _setCustomer: (state, action) => {
            ...
            const customer = state.customers.find((customer) => customer.customerid === action.payload)
        ...
        _editSelectProduct: (state, action) => {
            state.selected.products = state.selected.products.map((product, index) => {
        ...
        _delSelectProduct: (state, action) => {
            ...
            state.selected.products = state.selected.products.filter((_, index) => index !== action.payload)
        },
        ...
        _delSelectSet: (state, action) => {
            ...
            state.selected.sets = state.selected.sets.filter((_, index) => index !== action.payload)
        },
        ...
        _changeUserType: (state, action) => {
            state.users = state.users.map((user) => {
                if (user.userid === action.payload.userid) user.usertype = action.payload.usertype
                return user
            })
        }
    },
