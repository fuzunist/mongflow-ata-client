...
export const getOrdersFromDB = async (access_token) => {
    ...
    try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/order`, {
            headers: {
...
export const updateOrderStatusInDB = async (access_token, order_id, products, order_status, stock_diff, product_attributes) => {
...
export const updateOrderStatusSetInDB = async (access_token, order_id, sets, order_status, stockDiffs, attributes) => {
    try {
...
export const updateOrderInDB = async (access_token, order_id, customerid, currency_code, products, sets, subtotal, total_with_tax, order_status) => {
    try {
        const { data } = await axios.patch(
            `${import.meta.env.VITE_API_ENDPOINT}/order/${order_id}`,
            {
...
export const delOrderFromDB = async (access_token, order_id) => {
    try {
        const { data } = await axios.delete(`${import.meta.env.VITE_API_ENDPOINT}/order/${order_id}`, {
            headers: {
                Authorization: `Bearer ${access_token}`
            }
        })
...
export const createOrderNumberInDB = async (access_token) => {
    try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_ENDPOINT}/order/order-number`, {
            headers: {
                Authorization: `Bearer ${access_token}`
            }
        })
...
export const delOrderNumberFromDB = async (access_token, counter) => {
    try {
        const { data } = await axios.delete(`${import.meta.env.VITE_API_ENDPOINT}/order/${counter}/order-number`, {
            headers: {
                Authorization: `Bearer ${access_token}`
            }
        })
